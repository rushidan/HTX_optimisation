...
int main()
{
    ...
    for(i=0; i<22; i++) //loop for selection of Di and Do ; not i<23 because then Ds selection would be invalid
    {
        for(j=i+1; j<23; j++) //loop for selection of Ds
        {
            while(Di[j]<Do[i])
            {
                j++;
            ...
            //Pressure Drop Check
            if((dPs > dPs_allow) || (dPt > dPt_allow))
            {
                continue;
            }
            printf("\n U = %lf W/m2K for Di = %.3lf mm, Do = %.3lf mm, Ds = %.3lf mm", U[u], Di[i], Do[i], Ds);
    ...
    //final inference
    printf("\n\nTherefore, Maximum Possible Overall Heat Transfer Co-efficient = %lf W/m2K", Umax);
    printf("\n\nTherefore, Most Optimized Design Dimensions:\n");
    printf("Di = %.3lf mm\nDo = %.3lf mm\nDs = %.3lf mm", Di[i_Umax], Do[i_Umax], Di[j_Umax]);
    printf("\nTotal Length = %lf m\n\n", NL_optimized);
